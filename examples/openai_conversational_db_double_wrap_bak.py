...
class ConversationHistory:
    ...
    def get_or_create_salt(self, phone_number: str) -> bytes:
        if phone_number not in self.salt_cache:
            self.salt_cache[phone_number] = self._fetch_or_create_salt(phone_number)
        return self.salt_cache[phone_number]

    def _fetch_or_create_salt(self, phone_number: str) -> bytes:
        with self.get_connection() as conn:
            conn.execute("PRAGMA key = '{}';".format(self.master_key))
            cursor = conn.cursor()
            cursor.execute(
    ...
    def derive_key(self, phone_number: str) -> bytes:
        if phone_number not in self.key_cache:
            self.key_cache[phone_number] = self._derive_key(phone_number)
        return self.key_cache[phone_number]

    def _derive_key(self, phone_number: str) -> bytes:
        salt = self.get_or_create_salt(phone_number)
        kdf = PBKDF2HMAC(
    ...
    def encrypt(self, phone_number: str, data: str) -> tuple[bytes, bytes]:
        key = self.derive_key(phone_number)
        aesgcm = AESGCM(key)
        nonce = os.urandom(12)
        ciphertext = aesgcm.encrypt(nonce, data.encode(), None)
        return ciphertext, nonce

    def decrypt(self, phone_number: str, ciphertext: bytes, nonce: bytes) -> str:
        key = self.derive_key(phone_number)
        aesgcm = AESGCM(key)
    ...
    def __setitem__(self, phone_number: str, value: List[Dict[str, Any]]):
        with self.get_connection() as conn:
            ...
            for message in value:
                encrypted_message, nonce = self.encrypt(
    ...
    def append(self, phone_number: str, message: Dict[str, Any]):
        with self.get_connection() as conn:
            conn.execute("PRAGMA key = '{}';".format(self.master_key))
            cursor = conn.cursor()
            encrypted_message, nonce = self.encrypt(phone_number, json.dumps(message))
            cursor.execute(
                "INSERT INTO conversations (phone_number, encrypted_message, nonce) VALUES (?, ?, ?)",
                (
                    phone_number,
        ...
        if phone_number in self.message_cache:
            ...
            self.message_cache[phone_number] = self.__getitem__(phone_number)
...
async def watch_phone_number(phone_number: str):
    ...
    """
    last_length = len(conversation_history[phone_number])
